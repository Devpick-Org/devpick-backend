...
```bash
# PostgreSQL + Redis + Spring Boot 앱 전체 기동 (DynamoDB는 AWS 관리형)
...
| DB (비정형) | DynamoDB | AWS 관리형 (AI 요약, 퀴즈, RAG, 이벤트 로그) |
| 캐시 | Redis | 7 |
| 웹서버 | Nginx | 최신 |
...
| ADR | 결정 | 상태 |
|-----|------|------|
| ADR-001 | PostgreSQL(구조화) + DynamoDB(AI JSON/이벤트) 분리 | **확정** |
...
| `docs/통신.md` | FastAPI ↔ Spring 서버 간 통신 스펙 (POST /internal/contents, /internal/quiz, /internal/refine, /internal/answer 등 계약) |
